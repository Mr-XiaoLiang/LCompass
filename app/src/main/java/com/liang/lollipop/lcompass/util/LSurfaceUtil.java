 ...
 * SurfaceView的集成管理类
 */
public class LSurfaceUtil implements SurfaceHolder.Callback,TextureView.SurfaceTextureListener{
    //绘制的SurfaceView
    ...
    public void withCamera2(Context context) throws CameraAccessException{
        ...
        withCamera = true;
        if(!PermissionsUtil.checkPermission(context,PermissionsUtil.CAMERA_)){
    ...
    private void calibrationCameraDirection(String cameraId, WindowManager windowManager){
        ...
        if(isSurfaceView){
            ...
            Size maxSize = getMaxSize(totalRotation,sizes,surfaceView.getWidth(),surfaceView.getHeight());
            if(maxSize!=null){
                surfaceHolder.setFixedSize(maxSize.getWidth(),maxSize.getHeight());
